...
display_help () {
/bin/cat <<- HELP_MSG
	This script is a wrapper around the cat command-line tool.
	It outputs the cached response body associated with the 
	given HTTP location.
	
	$usage_string
	
	This script does not issue an HTTP request of any kind,
	and so the resource may or may not be fresh.
...
usage_string="Usage: $script_name [-hz] HTTP_LOCATION"
...
/bin/cat $( cache_response_body_file $compression_opt -d "$CACHE_DIR" "$location" )
exit $?
